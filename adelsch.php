...
if (isset($_GET['sch_id'])) {
    $result = $conn->prepare('SELECT * FROM `scholar` WHERE sch_id = ?');
    $result->execute([$_GET['sch_id']]);
    $user = $result->fetch(PDO::FETCH_ASSOC);

    $result = $conn->prepare('DELETE FROM `scholar` WHERE sch_id  = ?');
    $result->execute([$_GET['sch_id']]);

    $auditlogin = $conn->prepare("INSERT INTO `audit`(role, username, action) VALUES(?,?,?)");
    $auditlogin->execute(["admin", $fetch_profile['username'], "delete scholar"]);
