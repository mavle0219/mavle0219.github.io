...
if (isset($_GET['mm_id'])) {
  $result = $conn->prepare('SELECT * FROM `medmiss` WHERE mm_id = ?');
  $result->execute([$_GET['mm_id']]);
  $user = $result->fetch(PDO::FETCH_ASSOC);

  $result = $conn->prepare('DELETE FROM `medmiss` WHERE mm_id  = ?');
  $result->execute([$_GET['mm_id']]);
  ...
  header('location: amedmissre.php');
}
